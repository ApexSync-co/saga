...
export const CartProvider = ({ children }) => {
  const [cartItems, setCartItems] = useState(() => {
    ...
    const savedCart = localStorage.getItem('kamya_cart');
    if (savedCart) {
      try {
  ...
  useEffect(() => {
    if (!user) {
      ...
      setCartItems([]);
      localStorage.removeItem('kamya_cart');
      return;
    }
    ...
    if (user && user.id) {
      ...
      const unsubscribe = onSnapshot(cartRef, (docSnap) => {
        if (docSnap.exists()) {
          ...
          localStorage.setItem('kamya_cart', JSON.stringify(cloudItems));
        } else {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('kamya_cart', JSON.stringify(cartItems));
