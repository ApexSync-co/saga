...
const ProductOverview = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-bg-primary pt-32 flex justify-center items-center">
        <div className="flex flex-col items-center gap-4">
          <div className="w-12 h-12 border-4 border-primary border-t-transparent rounded-full animate-spin"></div>
          <div className="text-text-dark/60 text-xl font-Poppins tracking-widest">LOADING COLLECTION...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="pt-28 pb-20 px-4 min-h-screen bg-transparent">
      ...
      <div className="w-[90vw] max-w-7xl mx-auto text-text-dark/60 text-sm font-medium p-4 mt-6">
        <Link to="/" className="hover:text-primary transition-colors">Home</Link>
        <span className="mx-2 text-text-dark/30">•</span>
        <span className="text-text-dark">Product Overview</span>
      </div>

      <div className="text-center mb-16 relative">
        <h1 className="text-text-dark font-Great_Vibes text-7xl md:text-8xl mb-2 tracking-wider drop-shadow-lg">Our Collection</h1>
        <div className="w-24 h-1 bg-primary mx-auto rounded-full opacity-60"></div>
        <p className="text-text-dark/80 mt-6 font-Poppins text-xl max-w-2xl mx-auto px-4 leading-relaxed">
      ...
      <div className="max-w-7xl mx-auto mb-8 px-4 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex overflow-x-auto w-full md:w-auto no-scrollbar gap-2 pb-2 md:pb-0">
          {['All', 'Bangles', 'Bracelets', 'Earrings', 'Necklaces', 'Pendants', 'Rings', 'Sets'].map(cat => (
             <button 
                ...
                className={`px-5 py-2 rounded-full whitespace-nowrap text-sm font-medium transition-all duration-300 border border-border-gold/20 ${filterCategory === cat ? 'bg-primary text-white shadow-[0_0_15px_rgba(201,149,58,0.3)]' : 'bg-bg-card/40 text-text-dark/60 hover:bg-bg-card/60'}`}
             >
               {cat}
             </button>
          ))}
        ...
        <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
            ...
            <select 
                ...
                className="bg-bg-card border border-border-gold/20 text-text-dark text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-primary/50 transition-colors cursor-pointer"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {filteredAndSortedProducts.map((product) => (
            <div 
              ...
              className="bg-bg-card/40 backdrop-blur-xl rounded-3xl overflow-hidden border border-border-gold/10 group h-full flex flex-col hover:border-primary/50 transition-all duration-700 hover:shadow-[0_0_50px_rgba(201,149,58,0.15)] cursor-pointer"
            >
              ...
              <div className="p-8 flex flex-col flex-grow bg-gradient-to-b from-transparent to-black/20">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-text-dark text-2xl font-Poppins font-medium line-clamp-1 group-hover:text-primary transition-colors tracking-tight">
                    {product.name}
                  </h3>
                  <p className="text-crimson font-bold text-xl font-Poppins whitespace-nowrap ml-4">
                    {product.price}
                  </p>
                </div>

                <p className="text-text-dark/70 text-sm font-Poppins leading-relaxed mb-6 line-clamp-3 group-hover:text-text-dark transition-colors">
                ...
                <div className="mt-auto pt-6 border-t border-white/5">
                  <button 
                    ...
                    className="w-full group/btn relative overflow-hidden bg-crimson text-white py-4 rounded-2xl font-bold transition-all duration-500 active:scale-[0.98] border border-border-gold/30 hover:border-primary shadow-lg"
                  >
                    ...
                    <div className="absolute inset-0 bg-gold transform translate-y-full transition-transform duration-500 group-hover/btn:translate-y-0"></div>
                  </button>
                </div>
              </div>
            </div>
