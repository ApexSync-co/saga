...
const TrackingDisplay = ({ trackingData, onClose }) => {
    ...
    return (
        <div className={'fixed inset-0 z-100 flex items-center justify-center p-4 transition-all duration-700 ' + (isVisible ? 'bg-bg-primary/95 backdrop-blur-sm' : 'bg-transparent')}>
            <div 
            ...
            <div className={'relative w-full transition-all duration-500 transform overflow-hidden max-h-[90vh] flex flex-col ' + (showExtendedDetails ? 'max-w-4xl' : 'max-w-xl') + ' ' + (isVisible ? 'scale-100 opacity-100' : 'scale-95 opacity-0')}>
                <div className='bg-bg-card border border-border-gold/20 shadow-2xl overflow-hidden flex flex-col h-full min-h-0'>
                    {/* Header - Fixed */}
                    <div className='flex justify-between items-center px-6 py-5 border-b border-border-gold/20 bg-bg-card shrink-0'>
                        <div className='space-y-0.5'>
                            <h2 className='text-2xl font-light font-Great_Vibes text-text-dark tracking-widest'>Track Acquisition</h2>
                            <p className='text-[8px] uppercase tracking-[0.5em] text-text-dark/20 font-mono'>Ref: {trackingData.id ? trackingData.id.slice(0, 14).toUpperCase() : ''}</p>
                        </div>
                        <button 
                            onClick={() => { setIsVisible(false); setTimeout(onClose, 500); }}
                    ...
                    <div className={'flex flex-col lg:flex-row flex-1 min-h-0 ' + (showExtendedDetails ? 'overflow-y-auto custom-scrollbar' : 'overflow-hidden')}>
                            ...
                            <div className='space-y-8'>
                                                    ...
                                                    <div key={index} className='flex flex-col items-center relative'>
                                                        ...
                                                        <div className='relative w-4 h-4 flex items-center justify-center'>
                                                            <div className={'w-2 h-2 rotate-45 border transition-all duration-700 ' + 
                                                                (isActive ? 'bg-primary border-primary' : 'bg-bg-card border-border-gold/30')}>
                                                            </div>
                                                            {isCurrent && (
                                ...
                                <div className='grid grid-cols-2 sm:grid-cols-3 gap-6 border-t border-white/5 pt-6 mt-0'>
                                    <div className='space-y-1'>
                                        <p className='text-[8px] uppercase tracking-[0.3em] text-text-dark/20 font-bold'>Status</p>
                                        <p className='text-[10px] font-light text-text-dark italic truncate'>{currentStatusLabel || 'In Transit'}</p>
                                    </div>
                                    <div className='space-y-1 text-center sm:text-center'>
                                        <p className='text-[8px] uppercase tracking-[0.3em] text-text-dark/20 font-bold'>Arrival</p>
                                        <p className='text-[10px] font-light text-text-dark truncate'>{trackingData.eta || trackingData.mockTracking?.eta || '4-5 Days'}</p>
                                    </div>
                                    <div className='space-y-1 text-left sm:text-right col-span-2 sm:col-span-1 border-t border-border-gold/20 pt-4 sm:pt-0'>
                                        <p className='text-[8px] uppercase tracking-[0.3em] text-text-dark/20 font-bold'>Terminal</p>
                                        <p className='text-[10px] font-light text-text-dark truncate'>{trackingData.location || trackingData.mockTracking?.location || 'Origin Hub'}</p>
                                    </div>
                                </div>
                        ...
                        {showExtendedDetails && (
                            <div className='lg:w-2/5 p-8 bg-bg-card/90 animate-in fade-in slide-in-from-right-10 duration-700 border-l border-border-gold/20'>
                                <h3 className='text-[9px] uppercase tracking-[0.5em] text-white/30 font-bold mb-8'>Logistics Log</h3>
                                
                                <div className='space-y-10'>
                                    <div className='space-y-4 relative'>
                                        <div className='space-y-1'>
                                            <p className='text-[8px] uppercase tracking-[0.4em] text-text-dark/10'>Last Transmission</p>
                                            <div className='flex items-center gap-2'>
                                                <p className='text-[11px] font-light text-text-dark tracking-wide uppercase'>{trackingData.status || 'Transit Confirmed'}</p>
                                                <span className='w-1 h-1 bg-primary rounded-full'></span>
                                                <p className='text-[9px] text-primary/60 font-mono italic'>{trackingData.location || 'ZRH Terminal'}</p>
                                            </div>
                                        </div>
                                        <div className='space-y-1 pt-4 border-t border-border-gold/20'>
                                            <p className='text-[8px] uppercase tracking-[0.4em] text-text-dark/10'>Chronograph</p>
                                            <p className='text-[10px] font-light text-text-dark/60 tracking-widest'>Updated {trackingData.lastUpdated ? new Date(trackingData.lastUpdated).toLocaleTimeString() : '14:22 GMT'}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className='space-y-8 pt-2 h-50 overflow-y-auto pr-2 custom-scrollbar'>
                                        {(trackingData.history || [
                                            ...
                                            { status: 'Manifest Created', location: 'KAMYA Treasury', time: '2 Days Ago' }
                                        ]).map((log, i) => (
                    ...
                    <div className='p-6 bg-bg-card border-t border-border-gold/20 shrink-0'>
                        <button 
                            ...
                            className={'w-full py-3.5 border text-[8px] uppercase tracking-[0.4em] font-bold transition-all duration-500 rounded-sm ' + 
                                (showExtendedDetails ? 'border-text-dark text-text-dark hover:text-primary hover:border-primary ' : 'border-border-gold/30 text-text-dark/20 hover:text-text-dark hover:border-primary bg-bg-card/40')}
                        >
