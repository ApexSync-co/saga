...
export default function Search({ isOpen, onClose }) {
    ...
    return (
        <div className="fixed inset-0 z-[100]">
            ...
            {/* Search Container */}
            <div className="relative bg-bg-primary border-b border-border-gold/30 shadow-2xl animate-in slide-in-from-top duration-300">
                <div className="max-w-7xl mx-auto px-4 md:px-6 h-20 flex items-center gap-2 md:gap-4">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#C9953A" className="w-6 h-6 shrink-0">
                    ...
                    <input
                        ...
                        type="text"
                        placeholder="Search..."
                        className="flex-1 bg-transparent text-text-dark text-lg md:text-xl placeholder:text-neutral-500 focus:outline-none font-[Poppins] min-w-0"
                        value={query}
                        onChange={(e) => setQuery(e.target.value)}
                    />
                    <button 
                        onClick={handleClose}
                        className="p-2 text-neutral-400 hover:text-text-dark transition-colors rounded-full hover:bg-black/10 shrink-0"
                    >
                ...
                {query && (
                    <div className="border-t border-border-gold/20 max-h-[70vh] overflow-y-auto bg-bg-primary/95 backdrop-blur-md">
                                ...
                                {results.map((item, index) => (
                                    ...
                                    >
                                        <div className="flex items-center gap-6">
                                            <span className="text-text-dark/20 font-Great_Vibes text-xl group-hover:text-primary transition-colors">
                                                {String(index + 1).padStart(2, '0')}
                                            </span>
                                            <h3 className="text-2xl md:text-3xl text-text-dark/60 font-[Poppins] font-light group-hover:text-text-dark group-hover:pl-4 transition-all duration-500 ease-out">
                                                {item.name}
                                            </h3>
                                        </div>
