...
export default function CategoryGrid(){
    ...
    return(
        <>
        <div className="mb-16">
        <div className="text-text-dark flex flex-col gap-2 text-center w-[80vw] m-auto mt-4">
        ...
        <span className="md:text-xl md:mt-2 opacity-80">Shop by Category</span>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 w-[95vw] md:w-[80vw] mx-auto my-6 auto-rows-[200px] md:auto-rows-[300px]">
            {categories.map((cat, index) => {
                ...
                return (
                    <div 
                        key={cat.handle} 
                        className={`cursor-pointer group overflow-hidden rounded-2xl relative shadow-lg border border-border-gold/20 ${spanClass}`} 
                    ...
                    >
                        ...
                        <div className={`absolute inset-0 bg-gradient-to-t from-bg-card/90 via-bg-card/20 to-transparent flex flex-col ${index === 0 ? 'justify-center items-center' : 'justify-end'} p-6 md:p-8`}>
                            ...
                            <h1 className={`${index === 0 ? 'font-Poppins text-2xl md:text-5xl uppercase tracking-widest font-medium text-text-dark text-center' : 'font-Poppins text-lg md:text-2xl uppercase tracking-widest font-medium text-text-dark'} transition-colors`}>
                                {cat.title}
                            </h1>
                            {index === 0 && (
