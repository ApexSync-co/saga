...
function Navbar(){
   ...
   useEffect(() => {
    ...
    return(
        <>
        <nav className={`flex justify-between items-center py-4 fixed top-0 w-full z-50 transition-transform duration-300 bg-bg-primary/95 backdrop-blur-sm shadow-md border-b border-border-gold/30 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`} >
            <ul className=" flex gap-10 mx-10 max-w-fit relative z-50">
                ...
                >
                    <span 
                        className={`h-[3px] w-6 bg-text-dark rounded transition-all duration-500 ease-in-out transform ${
                            isOpen ? "rotate-45 translate-y-[9px]" : ""
                        }`}
                    ></span>
                    <span 
                        className={`h-[3px] w-6 bg-text-dark rounded transition-all duration-500 ease-in-out ${
           ...
           <Link to="/" className="flex items-center justify-center group">
                <div className="relative">
                    <img 
                        src="/kamya_logo.png" 
                        alt="" 
                        aria-label="Kamya Home" 
                        className='md:h-20 h-16 relative z-10 group-hover:scale-110 transition-transform duration-500'
            ...
            <div className="flex text-primary gap-6 md:mx-10 mx-4">
                <svg onClick={() => setIsSearchOpen(true)} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#C9953A" className="w-6 h-6 cursor-pointer hover:scale-110 hover:text-primary/80 transition-all">
                    <path strokeLinecap="round" strokeLinejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
                </svg>
                <Link to="/cart" className="relative group">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="#C9953A" className="w-6 h-6 cursor-pointer hover:scale-110 hover:text-primary/80 transition-all group-hover:fill-primary/10">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
                    </svg>
                    {cartCount > 0 && (
                        <span className="absolute -top-2 -right-2 border-2 border-bg-primary bg-primary text-white text-[10px] font-bold rounded-full w-5 h-5 flex items-center justify-center animate-pulse">
                            {cartCount}
                        </span>
                    )}
                </Link>
        ...
        <Search isOpen={isSearchOpen} onClose={() => setIsSearchOpen(false)} />
            ...
            <div style={{ backgroundColor: "#FAF3E0" }} className={`fixed top-0 left-0 h-screen h-[100dvh] w-[70vw] md:w-[30vw] bg-bg-card text-text-dark transform transition-transform duration-700 ease-in-out z-[100] flex flex-col shadow-2xl ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
               ...
               <div className="flex flex-col items-center mt-16 px-6 text-center pb-4 border-b border-primary/20">
                    {isAuthenticated ? (
                       <div className="flex flex-col items-center w-full">
                            ...
                            <p className="text-text-dark font-semibold text-lg">Hello, {user?.name || 'User'}</p>
                            <p className="text-text-dark/50 text-xs mb-4 break-all">{user?.email}</p>
                       </div>
                    ) : (
                       <>
                                                 <img 
                             ...
                             style={{ mixBlendMode: 'darken' }}
                         />
                        <p className="text-text-dark/70 text-sm font-semibold mb-2 max-w-[200px]">
                            Discover Kamya Elegance
                        </p>
                       </>
                    )}
               </div>
               <ul className="flex flex-col text-text-dark/80 overflow-y-auto no-scrollbar py-6">
                    <div className="w-full mb-6">
                        ...
                        <div className="pl-[72px] flex flex-col gap-3 mt-1 pb-2">
                             <Link to="/necklaces" onClick={() => setIsOpen(false)} className="text-xs text-text-dark/50 hover:text-primary transition-colors uppercase tracking-wider">Necklaces</Link>
                             <Link to="/bangles" onClick={() => setIsOpen(false)} className="text-xs text-text-dark/50 hover:text-primary transition-colors uppercase tracking-wider">Bangles</Link>
                             <Link to="/earrings" onClick={() => setIsOpen(false)} className="text-xs text-text-dark/50 hover:text-primary transition-colors uppercase tracking-wider">Earrings</Link>
                             <Link to="/pendants" onClick={() => setIsOpen(false)} className="text-xs text-text-dark/50 hover:text-primary transition-colors uppercase tracking-wider">Pendants</Link>
                             <Link to="/rings" onClick={() => setIsOpen(false)} className="text-xs text-text-dark/50 hover:text-primary transition-colors uppercase tracking-wider">Rings</Link>
                             <Link to="/bracelets" onClick={() => setIsOpen(false)} className="text-xs text-text-dark/50 hover:text-primary transition-colors uppercase tracking-wider">Bracelets</Link>
                             <Link to="/sets" onClick={() => setIsOpen(false)} className="text-xs text-text-dark/50 hover:text-primary transition-colors uppercase tracking-wider">Sets</Link>
                        </div>
                    </div>
