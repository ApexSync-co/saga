    ...
    return(
        ...
        <Search isOpen={isSearchOpen} onClose={() => setIsSearchOpen(false)} />
            ...
            <div style={{ backgroundColor: "black" }} className={`fixed top-0 left-0 h-screen h-[100dvh] w-[70vw] md:w-[30vw] bg-black text-white transform transition-transform duration-700 ease-in-out z-[100] flex flex-col shadow-2xl ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
               ...
               <div className="flex flex-col items-center mt-12 px-6 text-center pb-4 border-b border-primary/20">
                    {isAuthenticated ? (
                       <div className="flex flex-col items-center w-full">
                            <div className="h-14 w-14 bg-gradient-to-br from-primary/80 to-primary rounded-full flex items-center justify-center text-white text-2xl font-bold mb-2 border-2 border-primary/40 shadow-lg">
                                {user?.name ? user.name.charAt(0).toUpperCase() : 'U'}
                            </div>
                            <p className="text-white font-semibold text-lg">Hello, {user?.name || 'User'}</p>
                            <p className="text-white/50 text-xs mb-1 break-all">{user?.email}</p>
                       </div>
                    ) : (
                       <>
               ...
               <ul className="flex flex-col text-white/80 overflow-y-auto no-scrollbar py-3">
                    <div className="w-full mb-3">
                        ...
                        <Link to="/" className="w-full" onClick={() => setIsOpen(false)}>
                            <li className="cursor-pointer py-2 w-full hover:text-white hover:bg-white/5 flex items-center justify-start pl-10 gap-4 group transition-all duration-200 border-l-4 border-transparent hover:border-white/30">
                        ...
                        <div className="pl-[72px] flex flex-col gap-1.5 mt-1 pb-1">
